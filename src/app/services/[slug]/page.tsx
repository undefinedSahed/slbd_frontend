...
export default async function ServiceDetails({
  ...
  const service = services.find(
    (service) => slugify(service.title) === resolvedParams.slug
  );
    ...
    <main>
      ...
      >
        ...
        <div className="container text-center lg:space-y-8 space-y-5">
          <h2 className="lg:text-4xl font-bold tracking-wide">
            {service?.title}
          </h2>
          <Link href="/contact">
      ...
      <section className="py-8 lg:py-20">
        <div className="container lg:space-y-10 space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-10 items-center">
            <p className="leading-relaxed lg:text-lg text-sm text-justify lg:text-start">
      ...
      <section className="py-8 lg:py-20">
        <div className="container lg:space-y-10 space-y-7">
          <div className="text-center space-y-5">
            <h2 className="lg:text-3xl text-xl font-bold tracking-wide underline underline-offset-[15px] text-primary">
              Usage Area
            </h2>
          </div>
            ...
            {service?.useArea.map((area) => (
              <div key={area.id} className="lg:space-y-5 space-y-2">
                <Image
                  ...
                  className="w-full aspect-video object-cover rounded-md"
                />
                <h3 className="lg:text-base font-semibold text-center">
                  {area?.title}
                </h3>
              </div>
            ))}
