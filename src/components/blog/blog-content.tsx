...
const fetchBlog = async (title: string): Promise<BlogResponse> => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/blogs/${encodeURIComponent(title)}`, {
        headers: {
    ...
    return (
        <article className="space-y-8">
            ...
            <div className="space-y-4">
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight">{blog.title}</h1>
                ...
                <p className="text-base sm:text-lg text-muted-foreground">{blog.description}</p>
            </div>
